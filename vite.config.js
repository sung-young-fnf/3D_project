...
const ENHANCE_PROMPT = `**작업 정의**: 이 이미지는 3D Gaussian Splatting 으로 렌더된 매장 씬이며, 박스 영역 편집으로 일부 물체가 재배치된 **편집 후 상태** 다. 이 작업은 새 이미지 생성이 아니라 **박스 목록에 지정된 영역 주변에 한정된 국소 디노이징 + 박스 와이어프레임 선 제거 + Fill 색상자 배경 교체** 다. 출력은 입력 이미지와 **99% 픽셀 수준으로 동일** 해야 하며, 박스 목록 영역 밖은 전혀 변경되지 않는다.
...
**Fill 색상자 안내 — 매우 중요**: 이미지에 **단색의 직사각형/육면체 색상자** (베이지·연갈색·녹색·회색 등 주변 벽/바닥과 비슷한 톤) 가 보일 수 있다. 물체 모양 실루엣의 색 패치일 수도 있다. 이것은 **박스 편집으로 이동된 물체의 원래 자리를 임시로 채운 Fill 영역** 이다.
- **실제 가구·책장·선반·상자·냉장고·가전 등이 아니다**. 절대로 가구로 해석해 그리거나 텍스처·문양·경첩·손잡이 등을 추가하지 말 것.
- **Fill 색상자는 출력에서 제거** 하고, 그 자리를 **주변 벽·바닥·가구 배경 질감으로 자연스럽게 교체** 한다.
- 판별법: 가장자리가 부자연스러울 정도로 깔끔하고 기하학적인 직사각형/육면체, 내부가 완전 단색, 그림자나 조명 변화 없음 → 이게 Fill 색상자.
...
1-a. **Fill 색상자 완전 제거 및 배경 교체**: 위 "Fill 색상자 안내" 의 단색 색상자/패치를 **출력에서 100% 제거** 한다. 그 자리는 **주변 벽·바닥·가구 질감** (톤·그림자·조명 모두 일치) 으로 자연스럽게 교체. 새 물체(책장·상자·가구 등) 로 해석·보강·장식하지 말 것. Fill 색상자 영역은 "이 자리에 원래 있던 배경" 처럼 보여야 한다.
...
- ❌ **Fill 색상자를 책장·선반·상자·가구로 오해해 텍스처/문양/경첩/손잡이 추가하기**
- ❌ **Fill 색상자를 그대로 유지해 단색 덩어리로 출력하기** — 반드시 주변 배경으로 교체
- ❌ Fill 색상자 경계에 그림자/하이라이트 추가해 입체감 부여

## 한 줄 요약

**박스 와이어프레임 선 제거 → Fill 색상자(단색 직사각형 패치) 배경 교체 → 박스 선 내부만 디노이징 → 박스 밖의 ghost(이동 잔재)는 배경으로 지움 → 그 외 픽셀은 원본 그대로. 같은 물체가 2개 선명하게 출력되면 실패. Fill 색상자를 가구(책장·선반)로 해석하면 실패. 당신의 창의력은 필요없다. 선·Fill·ghost 제거 + 국소 노이즈 제거만 하라.**`;

const SEGMENT_PROMPT = `**작업**: 이 이미지에서 "{target}" 에 해당하는 물체의 **픽셀 단위 이진 마스크** 를 생성하라.

**출력 요구사항**:
- 입력 이미지와 **완전히 동일한 해상도·구도·프레이밍** 의 이미지를 출력.
- "{target}" 에 해당하는 픽셀은 **순수 흰색 (RGB 255,255,255, 값=1.0)**.
- 그 외 모든 픽셀 (배경·다른 물체·바닥·그림자) 은 **순수 검정 (RGB 0,0,0, 값=0.0)**.
- **회색·그라디언트·앤티앨리어싱 금지** — 오직 흑 또는 백 두 가지 색만 존재.
- **경계 부드럽게 하지 말 것** — 물체 테두리는 **날카로운 계단식 픽셀 경계** (hard edge). 1픽셀 범위의 blur/그라디언트도 금지. 모든 픽셀이 정확히 0 또는 255 중 하나.
- **물체 내부 구멍 금지** — 물체 영역 내부는 전부 흰색 (가운데에 검정 픽셀 섞이지 말 것).
- 물체의 정확한 윤곽을 픽셀 단위로 표시. 직사각형 근사 금지.
- **물체보다 약간 크게 잡아도 됨** — 경계에서 한두 픽셀 여유는 허용. 축소해서 잘리는 것보다 낫다.
    ...
    configureServer(server) {
      ...
      server.middlewares.use("/api/segment", async (req, res, next) => {
        ...
        try {
          ...
          const geminiResp = await withTimeout(geminiCall, 60000, "Gemini");
